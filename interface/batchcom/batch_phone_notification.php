...
echo htmlspecialchars("<br>Total ".count($db_patient)." Records Found\n", ENT_QUOTES);
...
function cron_InsertNotificationLogEntry($prow,$phone_msg,$phone_gateway)
{
	...
	//Added by Yijin 1/12/10. Need to escape characters in message string which is likely to contain 's as are common in English
	//Changed to calling the function in formdata.inc.php
	$message = formAddSlashes($phone_msg);
	...
	$sql_loginsert .= "(NULL , '$prow[pid]', '$prow[pc_eid]', '$message', 'Phone', '$patient_info', '$phone_gateway', '$prow[pc_eventDate]', '$prow[pc_endDate]', '$prow[pc_startTime]', '$prow[pc_endTime]', '".date("Y-m-d H:i:s")."')";
